...
namespace procdraw_test {
    ...
    bool FunctionSpecChecker::Check(const char *filename, int expectedNumTests)
    {
        messages_.clear();
        int numTests = 0;
        ...
        if (!allPassed) {
            ...
            if (expectedNumTests != numTests) {
                messages_.push_back("EXPECTED "
                                    + std::to_string(expectedNumTests)
                                    + " TESTS BUT "
    ...
    std::vector<std::string> FunctionSpecChecker::GetMessages()
    {
        return messages_;
    }

    bool FunctionSpecChecker::CheckExample(tinyxml2::XMLElement *example, procdraw::LispInterpreter *L, int *numTests)
    {
            ...
            if (expectedValue != NULL) {
                ...
                *numTests = *numTests + 1;
                if (result == expectedValue) {
                    ...
                    testPassed = false;
                    messages_.push_back(std::string("EXPR: ") + expr
