...
static SpecInfo functionSpecs[] = {
    SpecInfo("clear.xml", 5),
    SpecInfo("cons.xml", 3),
    SpecInfo("difference.xml", 5),
    SpecInfo("eq.xml", 7),
    SpecInfo("functionp.xml", 4),
    SpecInfo("get.xml", 5),
    SpecInfo("keys.xml", 4),
    SpecInfo("make-dict.xml", 5),
    SpecInfo("memb.xml", 6),
    SpecInfo("product.xml", 5),
    SpecInfo("put.xml", 5),
    SpecInfo("quotient.xml", 4),
    SpecInfo("sum.xml", 5)
};
...
{
    bool passed = checker_.Check(ResolveSpecFile(GetParam().first).c_str(), GetParam().second);
    EXPECT_TRUE(passed);
    if (!passed) {
...
INSTANTIATE_TEST_CASE_P(CheckFunctionSpecs, FunctionSpecsTest, ::testing::ValuesIn(functionSpecs));
