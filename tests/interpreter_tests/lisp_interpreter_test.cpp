...
TEST_F(LispInterpreterTest, GetWithEmptyDictionary)
{
    ...
    EXPECT_EQ(
        100, L_.NumVal(L_.Get(L_.SymbolRef("key1"), dict, L_.MakeNumber(100))));
...
TEST_F(LispInterpreterTest, GetAndPutWithStringKeys)
{
    ...
    EXPECT_EQ(
        42, L_.NumVal(L_.Put(L_.MakeString("key1"), L_.MakeNumber(42), dict)));
    EXPECT_EQ(42, L_.NumVal(L_.Get(L_.MakeString("key1"), dict)));
    EXPECT_EQ(
        10, L_.NumVal(L_.Put(L_.MakeString("key1"), L_.MakeNumber(10), dict)));
...
TEST_F(LispInterpreterTest, EvalSetqInLambdaModifiesEnvironment)
{
    ...
    L_.Eval(L_.Read(
        "(setq f (lambda (a) (progn (setq b a) (setq a 3) (setq c a))))"));
...
{
    EXPECT_EQ(2, L_.NumVal(L_.Eval(
                     L_.Read("(apply (lambda (n) (+ n 1)) (quote (1)))"))));
}
