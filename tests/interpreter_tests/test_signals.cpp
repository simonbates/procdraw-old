...
TEST_CASE("Signals")
{
  ...
  L.Eval(L.Read("(setq step-incr (lambda (self) (put-slot self 'out (+ "
  ...
  SECTION("sin-osc")
  {
    ...
    REQUIRE(procdraw::ApproximatelyEqual(
      L.NumVal(L.Eval(L.Read("(sigval sin1)"))), 0.5, 0.01));

    L.Eval(L.Read("(put-slot sin1 'freq (/ 4))"));
    L.Eval(L.Read("(clear-stepped-signals)"));
    REQUIRE(procdraw::ApproximatelyEqual(
      L.NumVal(L.Eval(L.Read("(sigval sin1)"))), 1, 0.01));
    L.Eval(L.Read("(clear-stepped-signals)"));
    REQUIRE(procdraw::ApproximatelyEqual(
      L.NumVal(L.Eval(L.Read("(sigval sin1)"))), 0.5, 0.01));
    L.Eval(L.Read("(clear-stepped-signals)"));
    REQUIRE(procdraw::ApproximatelyEqual(
      L.NumVal(L.Eval(L.Read("(sigval sin1)"))), 0, 0.01));
    L.Eval(L.Read("(clear-stepped-signals)"));
    REQUIRE(procdraw::ApproximatelyEqual(
      L.NumVal(L.Eval(L.Read("(sigval sin1)"))), 0.5, 0.01));
    L.Eval(L.Read("(clear-stepped-signals)"));
