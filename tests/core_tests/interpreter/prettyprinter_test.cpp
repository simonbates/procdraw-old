...
TEST_F(PrettyPrinterTest, PrintLambdaFitParamsAndBreaksBetweenBodyForms)
{
  ...
  std::string expectedLongParamList = "(lambda (param1 param2 param3 param4)\n"
                                      "  (call1)\n"
                                      "  (call2 arg1 arg2 arg3))";
...
TEST_F(PrettyPrinterTest, PrintSigvalForm)
{
  EXPECT_EQ(
    "$foo",
    prettyPrinter_.PrintToString(
      &L_, L_.MakeList({ L_.SymbolRef("sigval"), L_.SymbolRef("foo") }), 100));
}
