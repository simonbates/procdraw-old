...
TEST(FunctionsTest, Product)
{
    Interpreter interpreter;
    EXPECT_EQ(1, interpreter.Eval(interpreter.Read("(*)")).GetInteger());
    EXPECT_EQ(0, interpreter.Eval(interpreter.Read("(* 0)")).GetInteger());
    EXPECT_EQ(2, interpreter.Eval(interpreter.Read("(* 2)")).GetInteger());
    EXPECT_EQ(6, interpreter.Eval(interpreter.Read("(* 2 3)")).GetInteger());
    EXPECT_EQ(24, interpreter.Eval(interpreter.Read("(* 2 3 4)")).GetInteger());
}
