...
double MapRange(double start1, double stop1, double start2, double stop2,
                double val);
double Norm(double start, double stop, double val);
int PowerOf2Gte(int n);
double Wrap(double start, double stop, double val);
glm::vec3 TriangleNormal(const glm::vec3 a, const glm::vec3 b,
                         const glm::vec3 c);
...
template <typename T>
void
SimpleMovingAverage<T>::AddDataPoint(T dataPoint)
{
  sum_ -= data_[index_];
  sum_ += dataPoint;
