...
TEST_CASE("LispInterpreter::MakeList()")
{
  ...
  SECTION("Empty list") { REQUIRE(L.Null(L.MakeList({}))); }
  ...
  {
    auto list1 = L.MakeList(
      { L.MakeNumber(1), L.MakeNumber(2), L.MakeNumber(3), L.MakeNumber(4) });
    REQUIRE(L.TypeOf(list1) == procdraw::LispObjectType::Cons);
  ...
  {
    auto list1 = L.MakeList({ L.MakeList({ L.MakeNumber(1), L.MakeNumber(2) }),
                              L.MakeNumber(3), L.MakeNumber(4) });
    REQUIRE(L.TypeOf(list1) == procdraw::LispObjectType::Cons);
...
TEST_CASE("LispInterpreter::PrettyPrintToString()")
{
  ...
  REQUIRE(L.PrettyPrintToString(L.MakeList({ L.MakeNumber(42) }), 100) ==
          "(42)");
}
...
TEST_CASE("LispInterpreter::Assoc()")
{
  ...
  SECTION("Non-empty association list")
  {
    ...
    auto alist = L.MakeList(
      { L.Cons(symbolA, L.MakeNumber(1)), L.Cons(symbolB, L.MakeNumber(2)) });
...
TEST_CASE("Dictionaries")
{
  ...
  SECTION("notFound is returned when get from empty Dictionary")
  {
    ...
    REQUIRE(L.NumVal(L.Get(L.SymbolRef("key1"), dict, L.MakeNumber(100))) ==
            100);
  ...
  SECTION("String keys")
  {
    ...
    REQUIRE(L.NumVal(L.Put(L.MakeString("key1"), L.MakeNumber(42), dict)) ==
            42);
    REQUIRE(L.NumVal(L.Get(L.MakeString("key1"), dict)) == 42);
    REQUIRE(L.NumVal(L.Put(L.MakeString("key1"), L.MakeNumber(10), dict)) ==
            10);
  ...
  SECTION("Keys")
  {
    ...
    auto foundExpectedKeys = (L.SymbolName(L.Car(twoKeys)) == "key1" &&
                              L.SymbolName(L.Cadr(twoKeys)) == "key2") ||
                             (L.SymbolName(L.Car(twoKeys)) == "key2" &&
                              L.SymbolName(L.Cadr(twoKeys)) == "key1");
...
TEST_CASE("LispInterpreter implicit type conversion")
{
  ...
  SECTION("to bool")
  {
    SECTION("should preserve booleans, convert nil to false, and other "
...
TEST_CASE("LispInterpreter::Eval")
{
  ...
  // For each input, call L.PrintToString(L.Eval(L.Read(input))) and check the
  // result.
  ...
  SECTION("NIL evaluates to itself") { REQUIRE(L.Null(L.Eval(L.Nil))); }
  ...
  SECTION("Eof evaluates to itself") { REQUIRE(L.IsEof(L.Eval(L.Eof))); }
  ...
  {
    auto env = L.MakeList({ L.Cons(L.SymbolRef("a"), L.MakeNumber(1)),
  ...
  {
    REQUIRE(L.PrintToString(L.Eval(L.Read("(lambda (n) (+ n 1))"))) ==
            "(lambda (n) (+ n 1))");
  }
  ...
  SECTION("SETQ in LAMBDA modifies the environment")
  {
    ...
    L.Eval(
      L.Read("(setq f (lambda (a) (progn (setq b a) (setq a 3) (setq c a))))"));
  ...
  {
    REQUIRE(
      L.NumVal(L.Eval(L.Read("(apply (lambda () (+ 1 2)) (quote ()))"))) == 3);
  }

  SECTION("APPLY 1 arg")
  {
    REQUIRE(L.NumVal(
              L.Eval(L.Read("(apply (lambda (n) (+ n 1)) (quote (1)))"))) == 2);
  }

  SECTION("APPLY 2 args")
  {
    REQUIRE(L.NumVal(L.Eval(L.Read(
  ...
  SECTION("IF")
  {
    ...
    {
      REQUIRE(L.NumVal(L.Eval(L.Read("(if (eq 42 42) (setq a 10) (setq b 20))"),
                              env)) == 10);
    ...
    {
      REQUIRE(L.NumVal(L.Eval(L.Read("(if (eq 42 42) (setq a 10))"), env)) ==
              10);
      REQUIRE(L.NumVal(L.Eval(L.Read("a"), env)) == 10);
      REQUIRE(L.NumVal(L.Eval(L.Read("b"), env)) == 2);
    }
    ...
    {
      REQUIRE(L.NumVal(L.Eval(L.Read("(if (eq 1 2) (setq a 10) (setq b 20))"),
                              env)) == 20);
  ...
  SECTION("MAP-RANGE")
  {
    ...
    SECTION("maps values from [0, 10] to [-1, 0]")
    {
      ...
      REQUIRE(L.NumVal(L.Eval(L.Read("(map-range 0 10 -1 0 (/ 10 4))"))) ==
              -0.75);
      REQUIRE(L.NumVal(L.Eval(L.Read("(map-range 0 10 -1 0 5)"))) == -0.5);
      REQUIRE(L.NumVal(L.Eval(L.Read("(map-range 0 10 -1 0 (/ 30 4))"))) ==
              -0.25);
    ...
    SECTION("maps values from [0, 10] to [1, -1]")
    {
      ...
      REQUIRE(L.NumVal(L.Eval(L.Read("(map-range 0 10 1 -1 (/ 10 4))"))) ==
              0.5);
      REQUIRE(L.NumVal(L.Eval(L.Read("(map-range 0 10 1 -1 5)"))) == 0.0);
      REQUIRE(L.NumVal(L.Eval(L.Read("(map-range 0 10 1 -1 (/ 30 4))"))) ==
              -0.5);
  ...
  {
    REQUIRE(L.PrintToString(L.Eval(L.Read("(assoc 'a '((a . 1)))"))) ==
            "(a . 1)");
    REQUIRE(L.PrintToString(L.Eval(L.Read("(assoc 'a '((b . 1) (a . 2)))"))) ==
            "(a . 2)");
    REQUIRE(L.PrintToString(L.Eval(
              L.Read("(assoc 'a '((b . 1) (a . 2) (a . 3)))"))) == "(a . 2)");
  }
  ...
  SECTION("PUTASSOC updates the first match, if one is found, and returns val")
  {
    ...
    REQUIRE(L.PrintToString(L.Eval(L.Read("alist"))) ==
            "((b . 1) (a . 4) (a . 3))");
  }
