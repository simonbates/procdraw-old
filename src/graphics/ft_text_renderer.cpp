...
void
FtTextRenderer::BeginText(int width, int height)
{
...
void
FtTextRenderer::DrawText(int x, int y, const TextLayout<GLfloat>& layout,
                         ...
                         TextLayout<GLfloat>::size_type endLineNum)
{
  ...
  for (std::remove_reference<decltype(layout)>::type::size_type i =
    ...
    glDrawArrays(GL_TRIANGLES, 0,
                 vertices.size() / FT_TEXT_RENDERER_COMPONENTS_PER_VERTEX);
  }
}

int
FtTextRenderer::GetLinespace()
{
  return asciiFontMetrics_.LinespacePixels;
}

TextLayout<GLfloat>
FtTextRenderer::LayOutText(const std::string& text, int maxLineWidthPixels)
{
...
void
FtTextRenderer::MakeGlyphQuadVao()
{
  glGenVertexArrays(1, &glyphQuadVao_);
  glBindVertexArray(glyphQuadVao_);

  auto vertexBufferSize = sizeof(GLfloat) * FT_TEXT_RENDERER_MAX_DRAW_GLYPHS *
  ...
  glVertexAttribPointer(0, FT_TEXT_RENDERER_COMPONENTS_PER_VERTEX, GL_FLOAT,
                        GL_FALSE, 0, 0);
...
void
FtTextRenderer::CalculateTextureSize(FT_ULong fromCharCode, FT_ULong toCharCode,
...
void
FtTextRenderer::RenderChar(FT_ULong charCode)
{
