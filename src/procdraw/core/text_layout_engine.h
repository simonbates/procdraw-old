...
#include "procdraw/core/bitmap_font_metrics.h"
...
template <typename T>
GlyphCoords TextLayoutEngine<T>::LayOutGlyph(
...
template <typename T> void TextLayoutEngine<T>::NewLine(TextLayout<T>* layout)
{
    layout->SetNumCharsInLine(numCharsThisLine);
    layout->OpenNewLine();
    cursorX = 0;
