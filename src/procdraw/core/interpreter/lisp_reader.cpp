#include "procdraw/core/interpreter/lisp_reader.h"
...
LispObjectPtr
LispReader::Read(LispInterpreter* L, const std::string& str)
{
  SetInput(str);
  return Read(L);
}
...
BalancedState
LispReader::CheckBalanced(const std::string& str)
{
  SetInput(str);
  int parenCount = 0;
...
bool
LispReader::IsStartOfNumber()
{
  return isdigit(ch_) != 0;
}

void
LispReader::GetNumber()
{
  std::string number;
  while (isdigit(ch_)) {
    number += ch_;
...
void
LispReader::Expect(LispTokenType t)
{
  if (token_ == t) {
    GetToken();
  } else {
...
LispObjectPtr
LispReader::ReadCons(LispInterpreter* L)
{
