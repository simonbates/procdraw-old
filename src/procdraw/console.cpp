...
namespace procdraw {
    ...
    void Console::Draw()
    {
        ...
        if (inputLineNeedsLayOut_) {
            inputLineLayout_ = renderer_->LayOutText(inputLine_.GetLine(), consoleWidth_);
    ...
    void Console::DrawConsoleText()
    {
        int y = 0;
        for (decltype(lines_)::size_type i = displayFromLogicalLine_; i < lines_.size(); ++i) {
            auto line = lines_.at(i);
            auto startLineNum = (i == displayFromLogicalLine_ ? displayFromPhysicalLine_ : 0);
    ...
    void Console::ScrollShowInput(int inputNumLines)
    {
        if (numVisibleLines_ + inputNumLines > lineCapacity_) {
            ...
            while (numVisibleLines_ > lineCapacity_ - inputNumLines) {
                if (displayFromPhysicalLine_ + 1 < lines_.at(displayFromLogicalLine_).layout.NumLines()) {
