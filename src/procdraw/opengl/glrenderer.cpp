...
#include "procdraw/core/math.h"
#include "procdraw/opengl/gl_util.h"
#include "procdraw/opengl/sdl_util.h"
#include "procdraw/opengl/vertex.h"
#include <iostream>
#include <string>
    ...
    , cubeVao_(0)
{
    ...
    textRenderer_
        = std::unique_ptr<BitmapTextRenderer>(new BitmapTextRenderer());
...
void GlRenderer::Begin2D()
{
    ...
    glUniformMatrix4fv(
        projection2dLoc_, 1, GL_FALSE, glm::value_ptr(projection));
...
void GlRenderer::DrawBlockCursorBackground(
...
void GlRenderer::DrawBlockCursorInversion(
    int cursorX, int cursorY, int cursorWidth, int cursorHeight)
{
    BeginInverse();
    Rect(cursorX, cursorY, cursorWidth, cursorHeight);
...
TextLayout<GLfloat> GlRenderer::LayOutText(
...
void GlRenderer::Rect(int x, int y, int w, int h)
{
    ...
    glUniform4f(
        materialColor2dLoc_, materialR_, materialG_, materialB_, materialA_);
    glBindVertexArray(rectangleVao_);
    glBindBuffer(GL_ARRAY_BUFFER, rectangleVertexBuffer_);
    glBufferSubData(
        GL_ARRAY_BUFFER, 0, sizeof(rectangleVertices_), rectangleVertices_);
...
void GlRenderer::RotateX(float turns)
{
    worldMatrix_ = glm::rotate(worldMatrix_,
        turns * 2 * static_cast<float>(M_PI), glm::vec3(1.0f, 0.0f, 0.0f));
}

void GlRenderer::RotateY(float turns)
{
    worldMatrix_ = glm::rotate(worldMatrix_,
        turns * 2 * static_cast<float>(M_PI), glm::vec3(0.0f, 1.0f, 0.0f));
}

void GlRenderer::RotateZ(float turns)
{
    worldMatrix_ = glm::rotate(worldMatrix_,
        turns * 2 * static_cast<float>(M_PI), glm::vec3(0.0f, 0.0f, 1.0f));
}
...
void GlRenderer::CompileShaders()
{
    static const GLchar* vertexShaderSource[]
        = { "#version 150                                                      "
            ...
            "        "
            " \n"
            "    vec4 diff_component = (light_color * material_color * "
            "diff_factor);   "
            " \n"
            "    vec4 ambient_component = (ambient_light_color * "
            "material_color);      "
            " \n"
...
void GlRenderer::MakeTetrahedronVao()
{
    ...
    Vertex vertices[]
        = { Vertex(p1, n1), Vertex(p2, n1), Vertex(p4, n1), Vertex(p1, n2),
            Vertex(p4, n2), Vertex(p3, n2), Vertex(p1, n3), Vertex(p3, n3),
            Vertex(p2, n3), Vertex(p2, n4), Vertex(p3, n4), Vertex(p4, n4) };
...
void GlRenderer::MakeRectangleVao()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, rectangleVertexBuffer_);
    glBufferData(GL_ARRAY_BUFFER, sizeof(rectangleVertices_),
        rectangleVertices_, GL_DYNAMIC_DRAW);
...
void GlRenderer::UpdateUniformsForObject()
{
    ...
    glUniform4fv(
        lightDirectionLoc_, 1, glm::value_ptr(modelSpaceLightDirection));
    ...
    glUniform4f(
        materialColorLoc_, materialR_, materialG_, materialB_, materialA_);
}
}
